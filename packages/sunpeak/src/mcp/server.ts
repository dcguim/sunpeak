 ...
 */
function readResourceHtmlProd(distPath: string, resourceName?: string): string {
  const htmlPath = path.resolve(distPath);

  if (!fs.existsSync(htmlPath)) {
    const label = resourceName ? `Resource "${resourceName}"` : 'Resource';
    throw new Error(
      `${label}: built HTML not found at ${htmlPath}. Run "sunpeak build" to generate it.`
    );
  }
...
<body><h1>Sunpeak MCP Server</h1><p>Connect via <a href="/mcp">/mcp</a></p></body>
  ...
  const onListening = () => {
    ...
    if (actualPort !== requestedPort) {
      console.log(
        `Sunpeak MCP server listening on http://localhost:${actualPort} (port ${requestedPort} was in use)`
      );
    } else {
