---
title: "ChatGPT Hooks"
description: "Host-specific React hooks for ChatGPT features: file uploads, modals, and checkout."
...
<Warning>
These hooks throw if called outside ChatGPT. Always feature-detect with [`isChatGPT()`](/api-reference/functions/host-detection) before using them.
</Warning>
...
When the ChatGPT host is selected in the [Inspector](/api-reference/simulations/inspector), a mock `window.openai` runtime is automatically injected into the iframe. This makes `isChatGPT()` return `true` and all ChatGPT hooks functional with stub implementations that log to the browser console.
...
<Card horizontal title="Inspector" icon="flask" href="/api-reference/simulations/inspector">
