...
<ResponseField name="enableJsonResponse" type="boolean" default="true">
...
Same shape as `ProductionServerConfig` (including `enableJsonResponse`, which defaults to `true`), except `auth` takes a Web Standard `Request`:
