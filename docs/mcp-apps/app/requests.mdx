...
<Note>
sunpeak provides convenience hooks for most of these methods: [`useCallServerTool`](/api-reference/hooks/use-call-server-tool), [`useSendMessage`](/api-reference/hooks/use-send-message), [`useUpdateModelContext`](/api-reference/hooks/use-update-model-context), [`useOpenLink`](/api-reference/hooks/use-open-link), [`useRequestDisplayMode`](/api-reference/hooks/use-request-display-mode), and [`useSendLog`](/api-reference/hooks/use-send-log). Use those instead of calling `app` methods directly in React components.
</Note>
