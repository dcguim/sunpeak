...
Hosts advertise MCP Apps support by including this key in their `extensions` capabilities during the MCP [initialization handshake](/mcp-apps/lifecycle#2-initialization). `getUiCapability` reads from this key internally.
