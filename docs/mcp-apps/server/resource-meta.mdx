...
Set `_meta.ui` on the resource config exported from your resource file. Sunpeak passes it through to both the [`resources/list`](/mcp-apps/mcp/resources#discovery-resourceslist) and [`resources/read`](/mcp-apps/mcp/resources#reading-resourcesread) responses automatically.

```ts
// src/resources/player/player.tsx
...
<Accordion title="Advanced: listing-level vs content-item (raw registerAppResource)">
When using [`registerAppResource`](/mcp-apps/server/register-app-resource) directly (without the Sunpeak framework), you can set `_meta` in two places:
